...
describe('research observation mode and precise reports', () => {
  ...
  it('shows a submitted run, lists cross-date activity, and confirms cancellation', async () => {
    ...
    const mockFetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes('/research/runs?')) return jsonResponse([{ run_id: 'old-run', trading_date: '2026-07-16', status: 'RUNNING', phase: '生成评分', progress: 55, trigger_source: 'AUTO' }])
      throw new Error(`unexpected request ${url}`)
    })
    vi.stubGlobal('fetch', mockFetch)
    render(researchWrapper(<ResearchPage />))
    ...
    const progress = screen.getByRole('progressbar', { name: '研究完成进度' })
    expect(progress).toHaveAttribute('aria-valuenow', '55')
    expect(progress.querySelector('.progress-runner')).not.toBeNull()
  ...
  it('reuses a successful NO_BUY plan while showing live market and frozen score details', async () => {
    ...
    expect(screen.getByText('当前不满足买入条件')).toBeInTheDocument()
    expect(screen.getByText('历史样本不足，暂不生成方案')).toBeInTheDocument()
    expect(screen.getByText('AI 说明不可用')).toBeInTheDocument()
