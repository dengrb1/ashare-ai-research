...
export function NotificationBell() {
  ...
  return <div className="notification-center" ref={panel}>
    ...
    {open && <section className="notification-panel" aria-label="通知中心">
      <header><div><strong>通知中心</strong><small>{summary.unread_count ? `${summary.unread_count} 条未读` : '暂无未读通知'}</small></div><div><button className="icon-button" disabled={!summary.unread_count} onClick={() => void markAllRead()} aria-label="全部标为已读" title="全部标为已读"><CheckCheck size={17} /></button><button className="icon-button" onClick={() => setOpen(false)} aria-label="关闭通知中心" title="关闭"><X size={17} /></button></div></header>
