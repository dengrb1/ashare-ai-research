...
export const api = {
  ...
  tradeAdviceMonitors: () => request<TradeAdviceMonitor[]>('/trade-advice-monitors'),
  saveTradeAdviceMonitor: (payload: Pick<TradeAdviceMonitor, 'symbol' | 'enabled'> & { manual_buy_price?: number | null; manual_sell_price?: number | null }, idempotencyKey = crypto.randomUUID()) => request<TradeAdviceMonitor>('/trade-advice-monitors', { method: 'PUT', headers: { 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(payload) }),
  ...
  deleteRun: (runId: string) => request<void>(`/runs/${encodeURIComponent(runId)}`, { method: 'DELETE' }),
  clearCompletedRuns: (before?: string, idempotencyKey = crypto.randomUUID()) => request<{ deleted: number; before?: string | null }>('/runs/clear-completed', { method: 'POST', headers: { 'Idempotency-Key': idempotencyKey }, body: JSON.stringify({ before: before || null }) }),
