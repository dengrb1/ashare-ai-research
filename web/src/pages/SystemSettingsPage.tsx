  ...
  return <div className="system-settings-page">
    ...
    <Panel title="系统资源" eyebrow={resources?.scope_label || 'RESOURCE MONITOR'} action={<span className={`resource-state ${stale ? 'stale' : resourceLevel.toLowerCase()}`}>{stale ? '数据陈旧' : resourceLevel}</span>}>
      {loading && !resources ? <Loading /> : <>
        ...
        <div className="resource-table desktop-only"><table><thead><tr><th>服务</th><th>角色</th><th>状态</th><th>工作集</th><th>可回收缓存</th><th>上限</th><th>CPU</th><th>采集时间</th></tr></thead><tbody>{resources?.services.map((service) => <tr key={service.service_id}><td><strong>{service.service_id}</strong></td><td>{service.role}</td><td><StatusPill status={service.healthy ? 'ACTIVE' : 'ERROR'} /></td><td>{bytes(service.memory_used_bytes)}</td><td>{bytes(service.memory_cache_bytes)}</td><td>{bytes(service.memory_limit_bytes)}</td><td>{service.cpu_percent == null ? '—' : `${service.cpu_percent.toFixed(1)}%`}</td><td>{formatTime(service.collected_at || resources.collected_at)}</td></tr>)}</tbody></table></div>
        <div className="resource-service-list mobile-only">{resources?.services.map((service) => <article key={service.service_id}><header><div><strong>{service.service_id}</strong><small>{service.role}</small></div><StatusPill status={service.healthy ? 'ACTIVE' : 'ERROR'} /></header><dl><div><dt>工作集</dt><dd>{bytes(service.memory_used_bytes)}</dd></div><div><dt>可回收缓存</dt><dd>{bytes(service.memory_cache_bytes)}</dd></div><div><dt>CPU</dt><dd>{service.cpu_percent == null ? '—' : `${service.cpu_percent.toFixed(1)}%`}</dd></div></dl></article>)}</div>
