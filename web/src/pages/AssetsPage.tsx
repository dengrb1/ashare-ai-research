...
const EMPTY_POSITION: PositionDraft = { symbol: '', name: '', quantity: '', cost: '' }

export function AssetsPage() {
  const { watchlist, positions, totalAssets, quotes, assetsLoading, assetsSaving, addWatch, removeWatch, reorderWatchlist, upsertPosition, removePosition, saveTotalAssets } = useMarket()
  ...
  function editPosition(position: PaperPosition) {
    setFormError('')
    setDraft({ symbol: position.symbol, name: position.name, quantity: String(position.quantity), cost: String(position.cost), previousSymbol: position.symbol })
  }

  async function savePosition() {
    if (!draft) return
    const position: PaperPosition = { symbol: draft.symbol.trim().toUpperCase(), name: draft.name.trim(), quantity: Number(draft.quantity), cost: Number(draft.cost) }
