    ...
    {loading ? <Loading label="正在读取盘中盈利研究" /> : <div className="exit-advice-layout">
      <Panel title="触发记录" eyebrow="PROFIT TRIGGERS">
        {rows.length ? <div className="advice-list">{rows.map((row) => <button key={row.advice_id} className={selected?.advice_id === row.advice_id ? 'active' : ''} onClick={() => setSelected(row)}>
          ...
          <div><StatusPill status={row.status} /><strong>{actionLabel(row.action)}</strong><small>浮盈 ¥ {formatAmount(Number(row.unrealized_profit))}</small></div>
      ...
      <Panel title="AI 分档卖出方案" eyebrow="EXIT LADDER">
        {!selected ? <Empty /> : <div className="page-stack">
          <div className="summary-grid compact"><div><span>当前价</span><strong>¥ {formatNumber(Number(selected.current_price))}</strong></div><div><span>触发线</span><strong>¥ {formatAmount(Number(selected.trigger_amount))}</strong></div><div><span>AI 结论</span><strong>{actionLabel(selected.action)}</strong></div><div><span>调用</span><strong>{selected.cache_hit ? '缓存命中' : '新生成'}</strong></div></div>
          {typeof result.summary === 'string' && <div className="advice-summary"><strong>研究结论</strong><p>{result.summary}</p></div>}
          {ladder.length ? <div className="table-wrap"><table><thead><tr><th>目标价</th><th>卖出数量</th><th>预计金额</th><th>状态</th><th>依据</th></tr></thead><tbody>{ladder.map((item, index) => <tr key={index}><td>¥ {item.target_price as string}</td><td>{String(item.quantity)} 股</td><td>¥ {formatAmount(Number(item.estimated_gross_proceeds))}</td><td>{item.status === 'READY_FOR_CONFIRMATION' ? '待用户确认' : '已阻断'}</td><td>{String(item.reason || '')}</td></tr>)}</tbody></table></div> : <Empty title="暂无可执行分档" />}
