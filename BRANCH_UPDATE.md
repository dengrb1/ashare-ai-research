...
4. **研究专用运行时**：统一为单一 `job-worker` 和轻量模式；Windows、Linux 原生控制器与 Compose 共享同一运行时契约。
...
- 默认 Compose 项目的 API、Web、模型网关、行情桥、PostgreSQL、Redis 和 `job-worker` 均为 healthy；Web `/` 与 API `/api/v1/health` 均返回 200。
