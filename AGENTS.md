...
- Docker 版、Windows 原生特供版和 Linux 原生特供版必须提供相同的 Edge Gateway 管理能力：FRP TOML、结构化 Nginx 代理主机、校验、版本、回滚、启停和应用状态不能只在某一种部署形态实现。
- Edge Gateway 管理界面必须读取部署对应配置目录中的外部文件变更，并在下一次读取或控制器轮询时导入版本；不能只读取数据库缓存。Docker 默认读取挂载的 `docker/edge-gateway`（敏感 `frpc.toml` 与生成的 `managed.conf` 不得提交），原生版统一读取 `<runtime>/config/edge-gateway`。
- 界面保存的配置必须通过受限控制器或原生进程写回对应配置目录，并使用原子替换、语法校验和 SHA-256 变更检测；文件目录不可写时必须保留数据库版本并明确显示待应用状态。
- 三种部署形态共用同一套 Pydantic/API 契约、FRP/Nginx 校验规则、资源上限和安全策略；不得为 Windows/Linux 添加绕过 API、认证、审计或代理目标限制的专用后门。
- 直接编辑配置文件与界面保存之间必须可互相发现：外部修改不能被静默覆盖，冲突时保留旧版本并拒绝应用，日志不得打印 FRP token 或完整敏感文件。
