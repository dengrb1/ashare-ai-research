...
登录后前端会把“自选股 + 我的持仓”合并去重，在后台调用行情预取接口。报价自动刷新频率按账户保存，可选 5/10/15/30/60/120 秒，默认 5 秒；手动刷新会先定向刷新当前选中证券并并行刷新可见 K 线，其他证券再在后台补全。未复权日 K 预热和缓存周期为 5 分钟；1/5/15/30/60 分钟线按需分段加载，最新分段最长缓存 30 秒并在页面停留、重新聚焦时自动检查。收盘后最后一根分钟线正常停在当日 15:00；后端会拒绝把上一交易日的数据标记为实时。实时行情缓存始终与研究、评分、报告和回测的不可变快照隔离。
...
并发请求腾讯日线并采用首个有效结果，分钟线仍保持 AKShare 主路径。
`POST /api/v1/market/prefetch` 单次最多接受 50 个去重后的标准代码，默认并发
数为 4；默认并行执行一批报价和逐股票未复权日 K 请求。传入 `include_quotes=false` 时只预热
日 K，供首页在报价请求完成后的空闲时段使用，避免重复读取报价。单个股票失败只写入 `errors`，不会
丢弃其他成功结果。该接口要求登录和 CSRF 请求头。AKShare 异常时，
无需密钥的新浪公开接口会提供未复权实时报价和分钟线，腾讯公开接口提供可选复权日线；配置
`TUSHARE_TOKEN` 后会作为额外备用。响应会标注实际来源，且只在所有上游均不可用时才返回
最近成功缓存并标记延迟。Web 默认按未复权（`raw`）契约返回分钟线和日线，使实时价与 K 线在除息后保持同一基准；显式请求 `adjust=hfq` 时返回后复权序列。实时行情服务不写入
